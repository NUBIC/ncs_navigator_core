      ...
      response_set.responses.sort_by { |r| r.question.display_order }.each do |r|
        ...
        if FATHER_PERSON_MAP.has_key?(data_export_identifier)
          unless value.blank?
            ...
            father.send("#{FATHER_PERSON_MAP[data_export_identifier]}=", value)
          end
        end
        ...
        if FATHER_ADDRESS_MAP.has_key?(data_export_identifier)
          unless value.blank?
            ...
            if father_address.nil?
              father_address = Address.new(:person => father, :dwelling_unit => DwellingUnit.new, :psu => person.psu, :response_set => response_set)
            end
            father_address.send("#{FATHER_ADDRESS_MAP[data_export_identifier]}=", value)
          end
        end
        ...
        if FATHER_PHONE_MAP.has_key?(data_export_identifier)
          unless value.blank?
            ...
            if father_phone.nil?
              father_phone = Telephone.new(:person => father, :psu => person.psu, :response_set => response_set)
            end
            father_phone.send("#{FATHER_PHONE_MAP[data_export_identifier]}=", value)
          end
        end
