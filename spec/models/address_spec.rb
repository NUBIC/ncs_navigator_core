...
describe Address do
  ...
  context "filtering addresses based on uniqueness and rank" do
    ...
    before do
      ...
      @uniquified_addresses = Address.unique_addresses(@addresses)
    end

    describe "#unique_addresses" do

      it "returns a set of unique addresses" do
        Address.unique_addresses(@addresses).should == [@address_a, @address_b, @secondary_home_type_address, @secondary_business_type_address]
      end
    ...
    describe "#highest_ranking_address" do

      it "returns single entries of highest rank(primary, secondary, duplicate, other) for a given type" do
        Address.highest_ranking_address(@uniquified_addresses).should == [@address_a, @address_b]
      end
    end
  end
