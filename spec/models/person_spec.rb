...
describe Person do
  ...
  context "filtering email addresses based on uniqueness and rank" do
    before do
      ...
      @uniquified_addresses = @person.unique_contact_mode_entries(@addresses)
    end

    describe "#unique_contact_mode_entries" do

      it "returns a set of unique addresses" do
        @person.unique_contact_mode_entries(@addresses).should == [@address_a, @address_b, @secondary_home_type_address, @secondary_business_type_address]
      end
    ...
    describe "#highest_ranking_contact_mode_entry" do
      ...
      it "returns single entries of highest rank(primary, secondary, duplicate, other) for a given type" do
        @person.highest_ranking_contact_mode_entry(@uniquified_addresses).should == [@address_a, @address_b]
      end
    end
  ...
  context "filtering email addresses based on uniqueness and rank" do
    ...
    before do
      ...
      @uniquified_email_addresses = @person.unique_contact_mode_entries(@email_addresses)
    end

    describe "#unique_contact_mode_entries" do
      it "returns a set of unique email addresses" do
        @person.unique_contact_mode_entries(@email_addresses).should == [@email_address_a, @email_address_b, @secondary_personal_type_email_address, @secondary_work_type_email_address]
      end
    ...
    describe "#highest_ranking_contact_mode_entry" do
      ...
      it "returns single entries of highest rank(primary, secondary, duplicate, other) for a given type" do
        @person.highest_ranking_contact_mode_entry(@uniquified_email_addresses).should == [@email_address_a, @email_address_b]
      end
    end
  ...
  context "filtering phone numbers based on uniqueness and rank" do
    ...
    before do
      ...
      @uniquified_phones = @person.unique_contact_mode_entries(@phones)
    end

    describe "#unique_contact_mode_entries" do
      it "returns a set of unique phones" do
        @person.unique_contact_mode_entries(@phones).should == [@phone_a, @phone_b, @secondary_home_phone, @secondary_work_phone]
      end
    ...
    describe "#highest_ranking_contact_mode_entry" do
      ...
      it "returns single entries of highest rank(primary, secondary, duplicate, other) for a given type" do
        @person.highest_ranking_contact_mode_entry(@uniquified_phones).should == [@phone_a, @phone_b]
      end
    end
