survey "INS_QUE_18MMother_INT_EHPBHI_P2_V1.0" do
  section "Interview introduction", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_1 "Insert date/time stamp", 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.TIME_STAMP_1"
    a :datetime
    
    label "Thank you again for agreeing to participate in the National Children’s Study. We are about to begin 
    the interview portion of today’s contact, which will take about 30 minutes to complete. Your answers are 
    important to us. There are no right or wrong answers. During this interview, we will ask about yourself, 
    your {CHILD/CHILDREN}, your health, where you live, and your feelings about being a part of the National 
    Children’s Study. You can skip over any questions or stop the interview at any time. We will keep 
    everything that you tell us confidential."
  end
  section "Interviewer-completed questions", :reference_identifier=>"18MMother_INT" do
    q_MULT_CHILD "Is there more than one child of this mother eligible for the 18-month visit today?",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.MULT_CHILD"
    a_1 "Yes"
    a_2 "No"
    ...
    # • IF MULT_CHILD = 1; 
    # • LOOP THROUGH QUESTIONNAIRE FOR EACH ELIGIBLE CHILD RECORDED IN CHILD_NUM THROUGH SMOKE_HOURS:
    # •  IF CHILD_QNUM>1: 
    ...
    # o SKIP TO TIME_STAMP_3.
    # o ASK THE FOLLOWING SECTIONS OF THE QUESTIONNAIRE: SLEEP, CHILD CARE ARRANGEMENTS, HEALTH CARE, MEDICAL CONDITIONS, MEDICATIONS, PRODUCT USE.
    # o SKIP TO SMOKE_HOURS.
    # o CONTINUE TO LOOP THROUGH QUESTIONAIRE FROM IN011 THROUGH SMOKE_HOURS FOR EACH CHILD UNTIL CHILD_NUM=CHILD_QNUM. THEN GO TO TIME_STAMP_10.
    # o LOOP THROUGH ROOM_MOLD_CHILD UNTIL CHILD_NUM=CHILD_QNUM. 

    q_CHILD_QNUM "Which number child is this questionnaire for?",
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_DETAIL.CHILD_QNUM"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_DETAIL.CHILD_SEX"
    a_1 "Male"
    a_2 "Female"
    a_3 "Both"
  ...
  section "Participant verification", :reference_identifier=>"18MMother_INT" do
...
#     TODO - the name should be pre-populated
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_DETAIL.CNAME_CONFIRM", :pick=>:one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_DETAIL.C_FNAME"
    a :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule=>"A"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_DETAIL.C_LNAME"
    a :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule=>"A"
    ...
    # TODO
    # PROGRAMMER INSTRUCTIONS: 
    # • IF PARTICIPANT REFUSES TO PROVIDE NAME, INITIALS OR IDENTIFIER C_FNAME AND C_LNAME= -1 OR -2, 
    # USE “YOUR CHILD” FOR “C_FNAME” IN REMAINDER OF QUESTIONNAIRE.
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_DETAIL.CDOB_CONFIRM"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    # TODO:
    #     PROGRAMMER INSTRUCTIONS:
    # • INCLUDE A SOFT EDIT/WARNING IF CALCULATED AGE IS LESS THAN 16 MONTHS OR GREATER THAN 22 MONTHS
    # • FORMAT CHILD_DOB AS YYYYMMDD
    # • IF CHILD_QNUM >1, GO TO TIME_STAMP_3
    
    q_PREGNANT "Are you pregnant now?",
    :help_text => "If adult is known to be pregnant, add [Just to confirm,]",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.PREGNANT"
    a_1 "Yes"
    ...
    label_PREG_LOSS "I’m so sorry for your loss. Please accept our sincere wishes at this difficult time"
    dependency :rule => "A"
  ...
  section "Demographics", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_2 "Insert date/time stamp", 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.TIME_STAMP_2"
    a :datetime
    
    q_MARISTAT "I’d like to ask about your marital status. Are you:",
    :help_text => "Record the participant’s current marital status.",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.MARISTAT"
    a_1 "Married,"
    ...
    q_MARISTAT_OTH "Other marital status", 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.MARISTAT_OTH"
    a "Specify", :string
    dependency :rule=>"A"
    condition_A :q_MARISTAT, "==", :a_neg_5
    
    q_HHCOMP_CHANGE "Have there been any changes in your household members since we contacted you last?",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.HHCOMP_CHANGE"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.HHCOMP_CHANGE_SPECIFY"
    a_text :text
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule=>"A"
    ...
    collect and is often used in scientific studies to compare groups of people who are similar. Please remember that 
    all the information you provide is confidential. Please think about your total combined family income during 
    [CURRENT YEAR – 1] for all members of the family."
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.HH_MEMBERS"    
    a_1 "Number", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don’t know"
    
    q_NUM_CHILD "How many of those people are children? Please include anyone under 18 years or anyone 18 years and 
    older and in high school.",
    :help_text => "Double check if the entry field for this question > than the answer above, or if responce is > 10",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NUM_CHILD"    
    a_1 "Number", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don’t know"
    ...
    q_INCOME "Of these income groups, which category best represents your total combined family income 
    during the last calendar year?",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.INCOME"
  ...
  section "Sleep", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_3 "Insert date/time stamp", 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.TIME_STAMP_3"
    a :datetime
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.SLEEP_HRS_DAY",
    :pick => :one    
    a "Hours", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.SLEEP_HRS_NIGHT",
    :pick => :one    
    a "Hours", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.SLEEP_DIFFICULT",
    :pick => :one    
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.SLEEP_THROUGH",
    :pick => :one
    a_1 "Never"
    a_2 "Occasionally"
    a_3 "Most nights"
  ...
  section "Child care arrangements", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_4 "Insert date/time stamp", 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.TIME_STAMP_4"
    a :datetime
    
    q_CHILDCARE_CHANGE "Has there been a change in your childcare arrangements since our last interview?",
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.CHILDCARE_CHANGE",
    :pick => :one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.CHILDCARE",
    :pick => :one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.FAMILY_CARE",
    :pick => :one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.FAMILY_CARE_HRS",
    :pick => :one
    ...
    condition_A :q_CHILDCARE, "==", :a_1
    condition_B :q_CHILDCARE_CHANGE, "==", :a_1    
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.HOMECARE",
    :pick => :one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    ...
    :help_text => "Please verify if response exceeds 50 hours per week",
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.HOMECARE_HRS",
    :pick => :one
    a "Number of hours per week", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.DAYCARE",
    :pick => :one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    ...
    :help_text => "Please verify if response exceeds 50 hours per week",
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.DAYCARE_HRS",
    :pick => :one
    a "Number of hours per week", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
  ...
  section "Health care", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_5 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.TIME_STAMP_5"
    a :datetime
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.R_HCARE"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.C_HEALTH"
    a_1 "Poor"
    a_2 "Fair"
    a_3 "Good"
    a_4 "Excellent"
    ...
    medical visits.",
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.USE_IC_LOG",
    :pick => :one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.REASON_NO_IC_LOG",
    :pick => :one
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.REASON_NO_IC_LOG_OTH"
    a "Specify", :string
    dependency :rule => "A"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.NUM_PROV_IC_LOG",
    :pick => :one
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.NUM_PROV_REC",
    :pick => :one
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.VISIT_WT",
    :pick => :one
    a "Pounds", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A or B or C"
    condition_A :q_USE_IC_LOG, "!=", :a_neg_7
    condition_B :q_USE_IC_LOG, "!=", :a_neg_1
    condition_C :q_USE_IC_LOG, "!=", :a_neg_2
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.HOSPITAL",
    :pick => :one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.HOSP_NIGHTS",
    :pick => :one
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.DIAGNOSE",
    :pick => :one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.DIAGNOSE_OTH",
    :pick => :one
    a "Diagnoses", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
  ...
  section "Medical conditions", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_6 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.TIME_STAMP_6"
    a :datetime
    ...
    :help_text => "Probe: \"Anything else?\". Select all that apply",
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.FEVER"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.ASTHMA",
    :pick => :one
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    ...
    walk, run, or play?",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.IHMOB"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"    
  end
  section "Medications", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_7 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.TIME_STAMP_7"
    a :datetime
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.PRESCR_TAKE"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    q_PRESCRMED "Please list the name of all prescription medications taken in the past 30 days:",
    :help_text => "Enter up to 10 medications; if more than 10 medications provided, enter first 10 provided by participant. 
    Probe: Anything else?",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_PRESCR.PRESCRMED"
    a :text
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A"
  ...
  # MED004B/(INTRO_PRESCRMED_2_10). Now let’s talk about the {PRESCRMED}.
    ...
    repeater "Information on prescription medicines:" do
      ...
      q "Interviewer instruction: What drug is first/next from the answer above?",
      :help_text => "Please pick the drug from the list above and put it in the answer. After the set of questions per selected drug 
      is answered, select \"Add Row\" to add the set of questions for the next drug"
      a :string

      q_PRESCR_ADMIN "How is the {PRESCRMED} taken?", 
      :pick => :one, 
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_PRESCR.PRESCR_ADMIN"
      dependency :rule => "A or B"
      condition_A :q_PRESCRMED, "!=", :a_neg_1
      condition_B :q_PRESCRMED, "!=", :a_neg_2
      a_1 "By mouth"
      ...
      :pick => :one, 
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_PRESCR.PRESCR_ADMIN_OTH"
      a "Specify", :string
      a_neg_1 "Refused"
      a_neg_2 "Don't know"
      dependency :rule => "A"
      condition_A :q_PRESCR_ADMIN, "==", :a_5

      q_PRESCR_TAKESTILL "Is {C_FNAME or YOUR CHILD} still taking the {PRESCRMED}?",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_PRESCR.PRESCR_TAKESTILL"
      a_1 "Yes"
      a_2 "No"
      a_neg_1 "Refused"
      a_neg_2 "Don't know"

      q_PRESCRIP_FREQ "How often does {C_FNAME or YOUR CHILD} use or take {PRESCRMED}?",
      :help_text => "Indicate frequency",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_PRESCR.PRESCRIP_FREQ"
      a "Enter number", :integer
      a_neg_1 "Refused"
      a_neg_2 "Don't know"
      dependency :rule => "A"
      condition_A :q_PRESCR_TAKESTILL, "==", :a_1

      q_PRESCRIP_FREQ_UNIT "Unit of how often does {C_FNAME or YOUR CHILD} use or take {PRESCRMED}?",
      :pick => :one,
      ...
      q_PRESCRIP_FREQ_ALT "How often did {C_FNAME or YOUR CHILD} use or take {PRESCRMED}?",
      :help_text => "Indicate frequency",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_PRESCR.PRESCRIP_FREQ"
      a "Enter number", :integer
      ...
      q_PRESCRIP_FREQ_UNIT_ALT "Unit of how often did {C_FNAME or YOUR CHILD} use or take {PRESCRMED}?",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_PRESCR.PRESCRIP_FREQ_UNIT"
      a_1 "Per day"
      a_2 "Per week"
    ...
    q_OTCMED "Please list the names of all non-prescription medications taken in the past 30 days:",
    :help_text => "Enter up to 10 medications; if more than 10 medications provided, enter first 10 provided by participant. 
    Probe: Anything else?",
    :pick => :one,
    ...
    repeater "Information on non-prescription medicines:" do
      ...
      q "Interviewer instruction: What drug is first/next from the answer above?",
      :help_text => "Please pick the drug from the list above and put it in the answer. After the set of questions per selected drug 
      is completed, select \"Add Row\" to add the set of questions for the next drug"
      a :string
      ...
      dependency :rule => "A or B"
      condition_A :q_OTCMED, "!=", :a_neg_1
      condition_B :q_OTCMED, "!=", :a_neg_2
      a_1 "By mouth,"
      a_2 "Inhaled either by mouth or nose,"
      a_3 "Injected,"
      a_4 "Applied to the skin, such as a patch or creams, or"
      ...
      :pick => :one, 
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_OTC.OTC_ADMIN_OTH"
      a "Specify", :string
      a_neg_1 "Refused"
      a_neg_2 "Don't know"
      dependency :rule => "A"
      condition_A :q_OTC_ADMIN, "==", :a_5

      q_OTC_TAKESTILL "Is {C_FNAME or YOUR CHILD} still taking the {OTCMED}?",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_OTC.OTC_TAKESTILL"
      a_1 "Yes"
      a_2 "No"
      a_neg_1 "Refused"
      a_neg_2 "Don't know"

      q_OTC_FREQ "How often does {C_FNAME or YOUR CHILD} use or take the {OTCMED}?",
      :help_text => "Indicate frequency",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_OTC.OTC_FREQ"
      a "Enter number", :integer
      a_neg_1 "Refused"
      a_neg_2 "Don't know"
      dependency :rule => "A"
      condition_A :q_OTC_TAKESTILL, "==", :a_1

      q_OTC_FREQ_UNIT "Unit of how often does {C_FNAME or YOUR CHILD} use or take the {OTCMED}?",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_OTC.OTC_FREQ_UNIT"
      a_1 "Per day"
      a_2 "Per week"
      a_3 "Per month"
      a_4 "Per year"
      ...
      q_OTC_FREQ_ALT "How often did {C_FNAME or YOUR CHILD} use or take the {OTCMED}?",
      :help_text => "Indicate frequency",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_OTC.OTC_FREQ"
      a "Enter number", :integer
      ...
      q_OTC_FREQ_UNIT_ALT "Unit of how often did {C_FNAME or YOUR CHILD} use or take the {OTCMED}?",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_OTC.OTC_FREQ_UNIT"
      a_1 "Per day"
      a_2 "Per week"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.SUPPL_TAKE"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    # • FIRST LOOP, DISPLAY INTRO_SUPPLMED_1. OTHERWISE DISPLAY SUPPLMED_2_10.
    ...
    q_SUPPLMED "Please list the names of all vitamins, minerals, herbals, and other dietary supplements taken in the past 30 days:",
    :help_text => "Enter up to 10 supplements; if more than 10 supplements provided, enter first 10 provided by participant. 
    Probe: Anything else?",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_SUPPL.SUPPLMED"
    a :text
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A"
    ...
    repeater "Information on supplements:" do
      ...
      :pick => :one, 
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_SUPPL.SUPPL_ADMIN"
      dependency :rule => "A or B"
      condition_A :q_SUPPLMED, "!=", :a_neg_1
      condition_B :q_SUPPLMED, "!=", :a_neg_2
      a_1 "By mouth,"
      a_2 "Inhaled either by mouth or nose,"
      a_3 "Injected,"
      a_4 "Applied to the skin, such as a patch or creams, or"
      ...
      :pick => :one, 
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_SUPPL.SUPPL_ADMIN_OTH"
      a "Specify", :string
      a_neg_1 "Refused"
      a_neg_2 "Don't know"
      dependency :rule => "A"
      condition_A :q_SUPPL_ADMIN, "==", :a_5

      q_SUPPL_TAKESTILL "Is {C_FNAME or YOUR CHILD} still taking the {SUPPLMED}?",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_SUPPL.SUPPL_TAKESTILL"
      a_1 "Yes"
      a_2 "No"
      a_neg_1 "Refused"
      a_neg_2 "Don't know"

      q_SUPPL_FREQ "How often does {C_FNAME or YOUR CHILD} use or take {SUPPLMED}?",
      :help_text => "Indicate frequency",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_SUPPL.SUPPL_FREQ"
      a "Enter number", :integer
      a_neg_1 "Refused"
      a_neg_2 "Don't know"
      dependency :rule => "A"
      condition_A :q_SUPPL_TAKESTILL, "==", :a_1

      q_SUPPL_FREQ_UNIT "Unit of how often does {C_FNAME or YOUR CHILD} use or take {SUPPLMED}?",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_SUPPL.SUPPL_FREQ_UNIT"
      a_1 "Per day"
      a_2 "Per week"
      a_3 "Per month"
      a_4 "Per year"
      ...
      q_SUPPL_FREQ_ALT "How often did {C_FNAME or YOUR CHILD} use or take {SUPPLMED}?",
      :help_text => "Indicate frequency",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_SUPPL.SUPPL_FREQ"
      a "Enter number", :integer
      ...
      q_SUPPL_FREQ_UNIT_ALT "Unit of how often did {C_FNAME or YOUR CHILD} use or take {SUPPLMED}?",
      :pick => :one,
      :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_SUPPL.SUPPL_FREQ_UNIT"
      a_1 "Per day"
      a_2 "Per week"
  ...
  section "Product use", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_8 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.TIME_STAMP_8"
    a :datetime
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.LICE_1"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :any,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_LICE.LICE_2"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.LICE_OTH_1"
    a "Specify: ", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A"
    condition_A :q_LICE_2, "==", :a_2

    # TODO
    # PROGRAMMER INSTRUCTION: 
    # • GO TO PROGRAMMER INSTRUCTIONS AT START OF “MATERNAL BEHAVIORS” SECTION.
    # Nataliya's comment - what does it mean??
    
    q_LICE_OTH_2  "Other: specify",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.LICE_OTH_2"
    a "Specify: ", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A"
    condition_A :q_LICE_2, "==", :a_3
  end 
  section "Maternal behaviors", :reference_identifier=>"18MMother_INT" do
    # TODO
    # PROGRAMMER INSTRUCTIONS: 
    ...
    q_time_stamp_9 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.TIME_STAMP_9"
    a :datetime
    dependency :rule=>"A"
    condition_A :q_CHILD_QNUM, "==", {:integer_value => "1"}

    label "The next questions are about your experiences in the past year. First, I’d like to ask some questions about 
    work. People’s work situations sometimes change after having a baby."
    dependency :rule=>"A"
    condition_A :q_CHILD_QNUM, "==", {:integer_value => "1"}
    
    q_WORK_LAST_CONTACT "Since our last interview, have you been employed at a job or business?",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.WORK_LAST_CONTACT"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.WORK_CURRENTLY"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.WORK_HRS"
    a_1 "Hours", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.R_SMOKE"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NUM_SMOKER"
    a "Number of smokers", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A and B"
    condition_A :q_R_SMOKE, "!=", :a_1
    condition_B :q_CHILD_QNUM, "==", {:integer_value => "1"}
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.SMOKE_RULES"    
    a_1 "No one is allowed to smoke anywhere inside my home,"
    a_2 "Smoking is allowed in some rooms at some times, or"
    ...
    or near enough that {he/she} can see or smell the smoke? Please consider all the places {C_FNAME or YOUR CHILD} is 
    during the day, including at home, at daycare, or some other place.",
    :help_text => "If {he/she} is not exposed to smoke, enter \"0\".",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_HABITS.SMOKE_HOURS"
    a "Hours", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    # • IF CHILD_NUM >1, GO TO CHILD_QNUM AND LOOP THROUGH QUESTIONAIRE FROM IN011 THROUGH SMOKE_HOURS 
    # FOR EACH CHILD UNTIL CHILD_NUM=CHILD_QNUM. THEN GO TO TIME_STAMP_10.
  end
  section "Pets", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_10 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.TIME_STAMP_10"
    a :datetime
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.PETS"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don’t know"
    ...
    :pick => :any,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_PET_TYPE.PET_TYPE"
    a_1 "Dog"
    a_2 "Cat"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_PET_TYPE.PET_TYPE_OTH"
    a_1 "Specify", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.PET_MEDS"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don’t know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.PET_MED_TIME"
  ...
  section "In-home exposures", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_11 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.TIME_STAMP_11"
    a :datetime
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.REMOVAL"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.METHOD"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.METHOD_OTH"
    a "Specify", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.OPEN_WINDOW"
    a_1 "Less than 1 hour per day,"
    a_2 "1-3 hours per day,"
    a_3 "4-12 hours per day,"
    a_4 "More than 12 hours per day, or"
    a_5 "Not at all?"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.ROACH"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don’t know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.WATER"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    inside your home?",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.MOLD"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :any,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_ROOM_MOLD.ROOM_MOLD"
    a_1 "Kitchen"
    a_2 "Living room"
    a_3 "Hall/landing"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_ROOM_MOLD.ROOM_MOLD_OTH"
    a "Specify", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A and B and C"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_MOLD.ROOM_MOLD_CHILD"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.RENOVATE"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick => :any,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_RENOVATE_ROOM.RENOVATE_ROOM"
    a_1 "Kitchen"
    a_2 "Living room"
    a_3 "Hall/landing"
    ...
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER_RENOVATE_ROOM.RENOVATE_ROOM_OTH"
    a "Specify", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A and B and C"
  ...
  section "Housing characteristics", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_12 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.TIME_STAMP_12"
    a :datetime
    ...
    :help_text => "If using showcards, refer participant to appropriate showcard. Otherwise, read response 
    categories to participant.",
    :pick => :one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.RECENT_MOVE"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick=>:one,
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.AGE_HOME"
    a_1 "2001 to present"
    a_2 "1981 to 2000"
    a_3 "1961 to 1980"
    a_4 "1941 to 1960"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.LENGTH_RESIDE"
    a "Number", :integer
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule=>"A"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.LENGTH_RESIDE_UNIT"
    a_1 "Weeks"
    a_2 "Months"
    a_3 "Years"
    a_neg_1 "Refused"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.WATER_DRINK"
    a_1 "Tap water,"
    ...
    q_WATER_DRINK_OTH "Other water source", 
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.WATER_DRINK_OTH"
    a_1 "Specify", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule=>"A"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.WATER_COOK"
    a_1 "Tap water,"
    a_2 "Filtered tap water,"
    a_3 "Bottled water, or"
    a_neg_5 "Some other source?"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.WATER_COOK_OTH"
    a_1 "Specify", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule=>"A"
  ...
  section "Neighborhood characteristics", :reference_identifier=>"18MMother_INT" do
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_DEFN"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_FAM"
    a_1 "None"
    a_2 "A few"
    a_3 "Many"
    a_4 "Most"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_FRIEND"
    a_1 "None"
    a_2 "A few"
    a_3 "Many"
    a_4 "Most"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGHBORS"
    a_1 "None"
    a_2 "A few"
    a_3 "Many"
    a_4 "Most"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_NUM_TALK"
    a_1 "None"
    a_2 "1 or 2"
    a_3 "3 to 5"
    a_4 "6 or more"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_HELP"
    a_1 "Often"
    a_2 "Sometimes"
    a_3 "Rarely"
    a_4 "Never"
    ...
    other on the street?",
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_TALK"
    a_1 "Often"
    a_2 "Sometimes"
    a_3 "Rarely"
    a_4 "Never"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_WATCH_1"
    a_1 "Very Likely,"
    a_2 "Likely,"
    a_3 "Unlikely,"
    a_4 "Very Unlikely"
    ...
    about it? Would you say it is...",
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_WATCH_2"
    a_1 "Very Likely,"
    a_2 "Likely,"
    a_3 "Unlikely,"
    a_4 "Very Unlikely"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_CLOSE"
    a_1 "Strongly agree,"
    a_2 "Agree,"
    a_3 "Disagree,"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_TRUST"
    a_1 "Strongly agree,"
    a_2 "Agree,"
    a_3 "Disagree,"
    a_4 "Strongly disagree"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_SAFE_1"
    a_1 "Strongly agree,"
    a_2 "Agree,"
    a_3 "Disagree,"
    a_4 "Strongly disagree"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_SAFE_2"
    a_1 "Strongly agree,"
    a_2 "Agree,"
    a_3 "Disagree,"
    a_4 "Strongly disagree"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.NEIGH_SAFE_3"
    a_1 "Strongly agree,"
    a_2 "Agree,"
    a_3 "Disagree,"
    a_4 "Strongly disagree"
  ...
  section "Tracing questions", :reference_identifier=>"18MMother_INT" do
    q_time_stamp_13 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.TIME_STAMP_13"
    a :datetime
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.COMM_EMAIL"
    a_1 "Yes"
    a_2 "No"
    a_3 "Don't remember"
    a_neg_1 "Refused"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.HAVE_EMAIL"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick=>:one, :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.EMAIL_2"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick=>:one, :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.EMAIL_3"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.EMAIL"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.COMM_CELL"
    a_1 "Yes"
    a_2 "No"
    a_3 "Don't remember"
    a_neg_1 "Refused"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CELL_PHONE_1"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick=>:one, :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CELL_PHONE_2"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CELL_PHONE_3"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CELL_PHONE_4"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CELL_PHONE"
    a_1 "Phone number", :string
    a_neg_7 "Participant has no cell phone"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"

    q_TIME_STAMP_14 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.TIME_STAMP_14"
    a :datetime
    ...
    :pick=>:one, :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.COMM_CONTACT"
    a_1 "Yes"
    a_2 "No"
    a_3 "Don't remember"
    a_neg_1 "Refused"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_1"
    a_1 "Yes"
    a_2 "No"
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    ...
    :pick=>:one, :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_FNAME_1"
    ...
    :help_text => "If participant does not want to provide name of contact ask for initials. Confirm spelling of first and last names", 
    :pick=>:one, :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_LNAME_1"
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_RELATE_1"
    a_1 "Mother/father"
    a_2 "Brother/sister"
    a_3 "Aunt/uncle"
    a_4 "Grandparent"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_RELATE1_OTH"      
    a_1 "Specify", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule=>"A"
    ...
    q_C_ADDR1_1 "Address 1 - street/PO Box", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_ADDR1_1"  
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_1, "==", :a_1

    q_C_ADDR_2_1 "Address 2", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_ADDR_2_1"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_1, "==", :a_1

    q_C_UNIT_1 "Unit", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_UNIT_1"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_1, "==", :a_1

    q_C_CITY_1 "City", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_CITY_1"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_1, "==", :a_1

    q_C_STATE_1 "State", :display_type=>"dropdown", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_STATE_1"
    a_1 "AL"
    a_2 "AK"
    a_3 "AZ"
    a_4 "AR"
    ...
    q_C_ZIPCODE_1 "ZIP Code", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_ZIPCODE_1"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_1, "==", :a_1

    q_C_ZIP4_1 "ZIP+4", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_ZIP4_1"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_1, "==", :a_1
    ...
    :pick=>:one, 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_PHONE_1"
    a_1 "Phone number", :string
    ...
    :help_text => "If participant does not want to provide name of contact ask for initials. Confirm spelling of first and last names", 
    :pick=>:one, :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_FNAME_2"
    a_1 "First name", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule=>"A"
    ...
    :help_text => "If participant does not want to provide name of contact ask for initials. Confirm spelling of first and last names", 
    :pick=>:one, :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_LNAME_2"
    a_1 "Last name", :string
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule=>"A"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_RELATE_2"
    a_1 "Mother/father"
    a_2 "Brother/sister"
    a_3 "Aunt/uncle"
    a_4 "Grandparent"
    ...
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_ADDR1_2"  
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_2, "==", :a_1

    q_C_ADDR_2_2 "Address 2", 
    :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_ADDR_2_2"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_2, "==", :a_1

    q_C_UNIT_2 "Unit", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_UNIT_2"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_2, "==", :a_1

    q_C_CITY_2 "City", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_CITY_2"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_2, "==", :a_1
    ...
    q_C_ZIPCODE_2 "ZIP Code", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_ZIPCODE_2"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_2, "==", :a_1

    q_C_ZIP4_2 "ZIP+4", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.C_ZIP4_2"
    a :string
    dependency :rule=>"A"
    condition_A :q_CONTACT_ADDR_2, "==", :a_1
    ...
    :help_text => "If contact has no telephone ask for telephone number where he/she receives calls", 
    :pick=>:one, :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.CONTACT_PHONE_2"
    a_1 "Phone number", :string
    ...
    q_hipv1_time_stamp_15 "Insert date/time stamp", :data_export_identifier=>"EIGHTEEN_MTH_MOTHER.TIME_STAMP_15"
    a :datetime 
  end    
end  
