survey "INS_QUE_24MMother_INT_EHPBHI_P2_V1.0" do
  ...
  section "Medications", :reference_identifier=>"24MMother_INT" do
    ...
    Probe: Anything else?",
    :pick => :one,
    :data_export_identifier=>"TWENTY_FOUR_MTH_MOTHER_PRESCR.PRESCR_TAKE"
    a :text
    a_neg_1 "Refused"
    a_neg_2 "Don't know"
    dependency :rule => "A"
...
# MED004B/(INTRO_PRESCRMED_2_10). Now let’s talk about the {PRESCRMED}.
    ...
    repeater "Information on prescription medicines:" do
      ...
      and select \"Add Row\" to add the set of questions for the next drug"
      a :string
    ...
    Include only those products purchased over the counter that do not require a prescription. [Do not include over-the-counter 
    ...
    repeater "Information on non-prescription medicines:" do
      ...
      q_OTC_FREQ_ALT "How often did {C_FNAME or YOUR CHILD} use or take {OTCMED}?",
      :help_text => "Indicate frequency",
      :pick => :one,
      :data_export_identifier=>"TWENTY_FOUR_MTH_MOTHER_OTC.OTC_FREQ"
      a "Enter number", :integer
      ...
      q_OTC_FREQ_UNIT_ALT "Unit of how often did {C_FNAME or YOUR CHILD} use or take {OTCMED}?",
      :pick => :one,
      :data_export_identifier=>"TWENTY_FOUR_MTH_MOTHER_OTC.OTC_FREQ_UNIT"
      a_1 "Per day"
      a_2 "Per week"
