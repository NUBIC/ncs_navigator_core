survey "INS_QUE_LIHIConversion_INT_HILI_P2_V1.0" do
  ...
  section "Outbound calling script", :reference_identifier=>"LIHIConversion_INT" do
    ...
    condition_B :q_OUT_ANSWER, "==", :a_neg_5
  ...
  section "Pregnancy probability group (PPG) script", :reference_identifier=>"LIHIConversion_INT" do
    ...
    # TODO - not displayed, yet has to be in place 
    q_APPT_1 "WHERE IS THAT APPT_1??????"
    a :string
    dependency :rule => "A"
    condition_A :q_q_FOLLOWUP_3, "==", :a_2
    
    # TODO - HOW DO WE CODE THIS????   
    # PPG004/(APPT_2).
    # PROGRAMMER INSTRUCTION: 
    # • TO SET PPG_STATUS. 
    # o USE PPG_STATUS FROM FOLLOWUP_1 OR FOLLOWUP_3  
    # o ELSE, SELECT THE MOST RECENT DATE OF PPG_FIRST OR PPG_STATUS =1 OR 2 => PPG_STATUS =4.
    # o IF PREGNANCY NOTIFICATION PPG_CONFIRM=1, THEN PPG_STATUS =2 => PPG_STATUS =1)]
    
    q_APPT_2 "How do we code this????????"
    a :string
    dependency :rule => "A or B"
    ...
    label_PPG005 "I’d like to go ahead and schedule a visit with you to talk about the next part of the Study. When would be a good time for you?",
    :help_text => "Set an appointment"
    dependency :rule => "A or B"
    condition_A :q_FOLLOWUP_1, "==", :a_3
    condition_B :q_FOLLOWUP_3, "==", :a_3
    ...
    :help_text => "End call and code case status."
    dependency :rule => "A or B or C "
    condition_A :q_FOLLOWUP_2, "==", :a_1
    condition_B :q_FOLLOWUP_2, "==", :a_neg_1
    condition_C :q_FOLLOWUP_2, "==", :a_neg_2
    ...
    dependency :rule => "A or B or C or D or E or F"
    condition_A :q_FOLLOWUP_1, "==", :a_1
    condition_B :q_FOLLOWUP_1, "==", :a_neg_1
    condition_C :q_FOLLOWUP_1, "==", :a_neg_2
    ...
    label_PPG010 "Thank you again for taking part in the Study. A member of our staff will be visiting with 
    you on [SCHEDULED APPOINTMENT DATE AND TIME] at [HOME, OUR OFFICE, OTHER PLACE]. If you have any questions 
    at all, please call our toll-free number, [TOLL-FREE NUMBER]. Thank you again. Goodbye.",
    :help_text => "End call. code case status."
